...
def compliance_view(request):
    """Module Fiscalité & Réglementaire — vue principale (Squelette)"""
    return render(request, 'compliance/index.html', {'page_title': 'Fiscalité & Réglementaire'})


@login_required
def api_compliance_data(request):
    """API: Get Compliance, Tax & Regulatory Data"""
    # Note: No models currently in compliance app, using static structure for now
    data = {
