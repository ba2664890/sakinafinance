"""
Django settings for SakinaFinance project.
...
if not SECRET_KEY:
    if DEBUG:
        ...
        raise ImproperlyConfigured(
            'DJANGO_SECRET_KEY must be defined when DEBUG is disabled.'
        )

raw_allowed_hosts = os.getenv('ALLOWED_HOSTS', 'localhost,127.0.0.1,0.0.0.0,.onrender.com')
ALLOWED_HOSTS = [host.strip() for host in raw_allowed_hosts.split(',') if host.strip() and host.strip() != '*']

# Security header for Render/Proxies
...
WSGI_APPLICATION = 'sakinafinance.wsgi.application'
...
ACCOUNT_EMAIL_VERIFICATION = 'none' # Disabled for testing on Render (Resend free tier restriction)
...
SOCIALACCOUNT_ADAPTER = 'sakinafinance.accounts.adapters.SakinaFinanceSocialAccountAdapter'
ACCOUNT_ADAPTER = 'sakinafinance.accounts.adapters.SakinaFinanceAccountAdapter'
...
DEFAULT_FROM_EMAIL = os.getenv('DEFAULT_FROM_EMAIL', 'SakinaFinance <onboarding@resend.dev>') # Warning: Update this when using custom domain
...
LOGGING = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': BASE_DIR / 'logs' / 'sakinafinance.log',
        },
        'console': {
            'level': 'INFO',
...
# Jazzmin Settings
JAZZMIN_SETTINGS = {
    "site_title": "SakinaFinance Admin",
    "site_header": "SakinaFinance",
    "site_brand": "SakinaFinance",
    "welcome_sign": "Bienvenue sur SakinaFinance Administration",
